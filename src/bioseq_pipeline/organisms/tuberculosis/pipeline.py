...
def write_snippy_commands(
    ...
    lines = [
        ...
        "# Run in Docker/Linux with snippy on PATH, or inside WSL after activating the Snippy environment.",
...
def write_snippy_readme(
    ...
    lines = [
        ...
        "",
        "## Как запустить в Docker/Linux/WSL",
        "",
        "```bash",
        "# Docker/Linux: Snippy is already on PATH in the project Docker image.",
        "# WSL: activate your local Snippy environment first if needed.",
...
def run_fasttree_wsl(
    ...
    local_fasttree = shutil.which(fasttree_exe)
    if local_fasttree:
        with tree_path.open("w", encoding="utf-8", newline="\n") as stdout, log_path.open("w", encoding="utf-8", newline="\n") as stderr:
            subprocess.run([local_fasttree, *flags, str(fasttree_alignment)], check=True, stdout=stdout, stderr=stderr)
    else:
        if not shutil.which("wsl"):
            raise ValueError(
                f"FastTree executable was not found on PATH: {fasttree_exe}. "
                "Install FastTree in the current environment or run this command inside the project Docker image."
            )
        ...
        bash_command = " ".join(
            [
                ...
                "&&",
                f"{shell_quote(fasttree_exe)} {flags_text}",
...
def add_prepare_inputs_arguments(parser):
    parser.add_argument(
        "--assembly-dir",
        type=Path,
        default=DEFAULT_ASSEMBLY_DIR,
...
def add_snippy_plan_arguments(parser):
    parser.add_argument(
        "--manifest",
        type=Path,
        default=DEFAULT_INPUT_MANIFEST_TSV,
...
def add_snippy_summary_arguments(parser):
    parser.add_argument(
        "--runs-dir",
        type=Path,
...
def add_build_tree_arguments(parser):
    parser.add_argument(
        "--alignment",
        type=Path,
        default=DEFAULT_SNIPPY_CORE_ALN,
    ...
    parser.add_argument(
        "--fasttree-exe",
        default="FastTree",
        help="FastTree executable name or path in the current environment or inside WSL fallback.",
    )
    parser.add_argument(
        "--no-gtr",
        action="store_true",
...
def add_mutation_summary_arguments(parser):
    parser.add_argument(
        "--runs-dir",
        type=Path,
        default=DEFAULT_SNIPPY_RUNS_DIR,
...
def add_itol_export_arguments(parser):
    parser.add_argument(
        "--sample-summary",
        type=Path,
        default=DEFAULT_SAMPLE_SUMMARY_TSV,
...
def add_report_arguments(parser):
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_SNIPPY_SUMMARY_REPORT,
        help="Snippy summary Markdown report. Kept for command-line compatibility.",
    )
    parser.add_argument(
        "--mutation-report",
        type=Path,
...
def add_tuberculosis_arguments(parser):
    ...
    subcommands = [
        ("metadata", "Build TB metadata/counts workbook.", add_metadata_arguments),
        ("prepare-inputs", "Prepare TB manifest, FASTA inputs, validation report, and H37Rv reference.", add_prepare_inputs_arguments),
        ("snippy-plan", "Create Docker/Linux/WSL Snippy command files from the prepared TB input manifest without running Snippy.", add_snippy_plan_arguments),
        ("snippy-summary", "Validate completed Snippy outputs and write summary workbook/report.", add_snippy_summary_arguments),
        ("build-tree", "Build a FastTree Newick tree from the Snippy core SNP alignment.", add_build_tree_arguments),
        ("mutation-summary", "Build assembly-based mutation summaries from per-sample Snippy snps.tab files.", add_mutation_summary_arguments),
        ("itol-export", "Create iTOL annotation datasets from mutation summaries and metadata.", add_itol_export_arguments),
        ("report", "Write final assembly-based tuberculosis analysis report.", add_report_arguments),
    ]
    for name, help_text, configure in subcommands:
...
def run_snippy_plan_command(args):
...
def run_snippy_summary_command(args):
...
def run_build_tree_command(args):
...
def run_mutation_summary_command(args):
...
def run_itol_export_command(args):
...
def run_prepare_inputs_command(args):
...
TB_COMMAND_HANDLERS = {
    "snippy-plan": run_snippy_plan_command,
    "snippy-summary": run_snippy_summary_command,
    "build-tree": run_build_tree_command,
    "mutation-summary": run_mutation_summary_command,
    "itol-export": run_itol_export_command,
    "report": run_report_command,
    "prepare-inputs": run_prepare_inputs_command,
    "metadata": run_metadata_command,
}
...
def run_from_args(args):
    ...
    handler = TB_COMMAND_HANDLERS.get(tb_command, run_metadata_command)
    handler(args)
